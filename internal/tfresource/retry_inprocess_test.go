...
func TestShortRetryDurationFunctionIsOperationLocal(t *testing.T) {
	...
	policy := GetRetryPolicy(false, "opensearch", override)
	if policy.MaximumNumberAttempts != maxAttemptsForRetryPolicy {
		t.Fatalf("operation-local policy attempts = %d, want %d", policy.MaximumNumberAttempts, maxAttemptsForRetryPolicy)
	}
}
